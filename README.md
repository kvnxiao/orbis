...
Orbis is a monorepo of modular [Pi](https://pi.dev/) agent extensions, published as `@orbis/*` npm
packages. Each package adds commands, tools, or event handlers to Pi and can be installed
independently.
...
Each package specification defines the behavior for an independent Pi implementation. Use the
available reference package or build from its `SPEC.md`. Specifications and source are distributed
...
- Compare strings and numbers explicitly in conditions, such as `name !== ""` or `count > 0`.
  Nullable object checks remain allowed.
