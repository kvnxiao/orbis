...
- [Explored alternatives](#explored-alternatives)
...
A **memory presentation** is the arrangement of committed work-note and index revisions in acting
context. A **presentation reset** removes superseded package-owned representations from that context
and establishes the latest valid state as its new baseline. It does not create a session, summarize
conversation, rewrite saved evidence, or count as Pi compaction.
...
The MVP appends complete revisions of changed current-work notes and bounded indexes. Earlier
representations remain at stable conversation positions until a presentation reset, compaction, or
validity change requires different context. A work-note update contains that note, not another full
compaction checkpoint. Supersession identifies the latest valid representation of each memory item;
it does not change source authority.

Prompt representation remains replaceable independently of observation, consolidation, canonical
memory, and recall. Canonical memory means the committed records and their validity, not the text
previously shown to the model. All representations share lineage, curation, authority, capacity, and
native-compaction guarantees. The MVP exposes only complete appended revisions; alternative runtime
modes and switching strategies during a live session are outside this MVP.
...
For an ordinary valid update, append the complete changed note with its revision, source boundary,
and an explicit statement that it supersedes prior work-note representations. Keep earlier
extension-controlled message content and positions unchanged; the later revision identifies those
notes as historical. An unchanged note does not cause another full copy merely because observation
coverage or unrelated metadata advanced. Preserve the latest source boundary in validated state and
expose any required boundary update without duplicating the unchanged body. Do not move the latest
note to the end of each request. Correctness after curation, invalidation, context edits, or lineage
changes takes precedence over prefix preservation.

Before dispatch, if accumulated package-owned work-note and index representations exceed their
documented prompt budget, reset their presentation. Remove identifiable superseded package-owned
copies from acting context, retain the complete latest valid note and the eligible bounded indexes,
and resume appending later changes from this baseline. The baseline stays at a stable position on
subsequent unchanged requests. Resetting must preserve retained user/assistant/tool conversation,
tool protocol pairs, known corrections, and required non-note checkpoint continuity. Preserve opaque
native checkpoint text whose memory components cannot be identified safely. Original session entries
and committed memory remain available. This deterministic reset requires no model call and may
change the cached prefix; it must not invoke compaction, alter native settings, or create a retry
loop. Apply the existing optional-memory removal and capacity stop when current state still cannot
fit.
...
Append the complete changed bounded index with its revision and supersession information. Keep
unchanged index content and placement stable and avoid duplicate copies. Index updates participate
in the same accumulated-presentation budget and reset as work-note revisions; they do not require
repeating an unchanged work note. Stable recall instructions must not be rewritten merely to publish
a memory revision.
...
After Pi commits a checkpoint, reconcile memory presentation against that checkpoint and the
retained conversation. Deduplicate only identifiable package-owned note or index representations,
and do not let a superseded revision in the retained tail regain current authority. Inclusion in a
native summarizer's input does not prove that its output preserved a memory revision. Preserve the
native checkpoint text; when its revision identity is unknown, do not suppress an otherwise valid
current revision on the assumption that the summary retained it. Preserve valid newer revisions and
corrections. A failed or cancelled compaction does not establish a new checkpoint baseline.
Presentation resets and compactions are distinct events and reported separately.
...
Reconstruct presentation baselines and emitted revision identities on the selected lineage as well
as canonical memory. Resume must not reappend an already represented revision or restore copies
suppressed by a presentation reset. A reset on an abandoned branch must not alter the selected
branch's presentation. Presentation bookkeeping does not advance evidence coverage or determine
whether a note is valid.
...
Bound accumulated work-note and index presentation separately from the single-note reserve and
complete checkpoint size. Count full rendered representations, supersession and boundary metadata,
and the baseline toward that budget. Use finite documented limits that permit the complete valid
current note; reset superseded presentation before applying the existing capacity stop. A reset does
not permit unbounded metadata growth or repeated resets on an unchanged baseline that cannot fit.
...
Report the selected prompt representation, its effective budget, estimated presentation tokens, and
presentation resets with their reason and before/after estimates. Preserve enough event records for
the evaluation runner to count resets across resume. Keep reset counts separate from compaction
counts. Provider-reported cache reads and writes, where available, remain distinct from estimated
prefix reuse; unavailable fields must remain unknown rather than being counted as zero savings.
...
| `REQ-current-work-note`, `REQ-recall-guidance`                                                          | Render a note and index, repeat unchanged requests, advance coverage without changing note content, then change the note and index independently.                                                                              | Existing extension-controlled prefix content and positions remain stable during ordinary appends; only the changed component gets a full revision, supersession and source boundaries remain explicit, and unchanged bodies are not duplicated.                                                                                                            | Fixture; Pi           |
| `REQ-current-work-note`, `REQ-resource-budgets`, `REQ-status-reporting`                                 | Append revisions until the presentation budget would be exceeded; repeat with a smaller model, native auto-compaction disabled, and an unchanged baseline that itself cannot fit.                                              | A deterministic reset removes only superseded package-owned presentation, preserves current mandatory state and conversation, changes the prefix only as needed, records its reason and size estimates, and resumes stable appends. Optional memory yields first; an impossible current note stops dispatch without a reset/compaction loop or model call. | Fixture; Pi           |
| `REQ-session-lineage`, `REQ-current-work-note`, `REQ-user-curation`, `REQ-activation-controls`          | Reset presentation, resume, fork or navigate before and after the reset, curate a note, disable memory, and perform native compaction.                                                                                         | The selected lineage reconstructs its presentation without duplicate emission or revival of invalid state. Original records remain retrievable, and native compaction can see durable supersession and invalidation even when acting-context filtering or new injection is disabled.                                                                       | Fixture; Pi           |
| `REQ-checkpoint-eligibility`, `REQ-current-work-note`, `REQ-recall-guidance`                            | Commit a custom or native checkpoint with memory revisions on both sides of the retained boundary; include an opaque native summary with unknown retained revision identity, then repeat with failed and cancelled compaction. | Only a committed checkpoint establishes its baseline; required continuity and newer valid revisions survive, superseded retained copies do not become current, and identifiable duplicates are not appended again. Opaque native text remains intact, and unknown identity does not suppress an otherwise valid current revision.                          | Fixture; Pi           |

## Explored alternatives

- Replacing the current snapshot on every update or appending only changes is deferred to post-MVP
  comparison. Complete appended revisions keep current state self-contained while preserving earlier
  prompt content between bounded resets; their cost and quality advantage remains unproven.
- Separate strategy packages are deferred until reuse warrants an external interface. Internal
  strategy modules share one owner for memory validity and Pi lifecycle integration.
- Stopping solely to preserve superseded prompt prefixes is rejected. A presentation reset permits
  bounded continuation; the mandatory-note capacity stop still applies when current state cannot
  fit.
- Replacing native fallback summarization or extending provider integration is deferred to a future
  complementary package or post-MVP work. The MVP preserves native preparation, triggers, and
  fallback.
...
Post-MVP prompt-representation comparisons can reuse the same canonical memory with complete
appended revisions, baseline-plus-change messages, or a replaced current snapshot. These variants
change presentation, not the protected-note architecture. Replay identical committed revisions to
compare representation size, stable prefixes, and reset behavior; separately compare actual agent
continuation, recall and evidence use, total cost, cache usage, and latency under matched initial
conditions. End-to-end runs may diverge after different prompts and must report those outcomes.
Record strategy parameters and budget/reset policy. These experiments do not add MVP production
modes or replace the required native-Pi comparison.
